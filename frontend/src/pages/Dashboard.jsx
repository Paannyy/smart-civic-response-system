...
const blankFilters = { status: "", category: "", priority: "" };
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (user.role === "citizen") {
        setComplaints(await getMyComplaints(token));
      } else if (user.role === "authority") {
        setComplaints(await getAssignedComplaints(token, filters));
      } else {
        const [allComplaints, allUsers] = await Promise.all([
          getAdminComplaints(token, filters),
          getUsers(token),
        ]);
        ...
        <section className="content">
          ...
          {view === "create" ? (
            ...
            <>
              ...
              {!citizen && <Filters filters={filters} setFilters={setFilters} />}
